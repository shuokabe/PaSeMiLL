...
def get_csls(se, te, knn, csls_knn, gpu=True):
    ...
    for sidx, sindices in enumerate(tqdm(st_indices)):
        ...
        for ti, tidx in enumerate(sindices):
            rs = rs_lst[tidx]
            csls_dists.append(2*st_distances[sidx][ti] - rt - rs) #original line
            #csls_dists.append(st_distances[sidx][ti] / (rt + rs))
...
def main(source_embeddings, binary, output, target_embeddings=None, knn=10,
    ...
    se = KeyedVectors.load_word2vec_format(source_embeddings, binary=binary, encoding='utf-8', unicode_errors='replace', limit=max_vocab)
    if target_embeddings is None or target_embeddings == source_embeddings:
        te = se
    else:
        te = KeyedVectors.load_word2vec_format(target_embeddings, binary=binary, encoding='utf-8', unicode_errors='replace', limit=max_vocab)
